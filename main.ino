...
#define URL_fw_Bin "https://raw.githubusercontent.com/Greenwend-Energy/firmware_version_update/main/3PEM.bin"
...
unsigned short CurrentGainCT1 = 31923;
unsigned short CurrentGainCT2 = 31923; 
unsigned short CurrentGainCT3 = 31923; 
unsigned short CurrentGainCT4 = 31923;
unsigned short CurrentGainCT5 = 31923; 
unsigned short CurrentGainCT6 = 31923;
/*
...
//Send Important Email From GWE Team.
const int BUTTON_PIN = 13; //  pin number of the button
const int LONG_PRESS_TIME = 300; //  represents the time (in milliseconds) that the button needs to be held down before it is considered a long press.
...
void setupAP(void)
{
  WiFi.mode(WIFI_AP); // The setupAP() function first sets the WiFi mode to station (WIFI_STA) 
...
// to create a web page to display the available networks.
  ...
  st += "</ol>";
  delay(100);
  WiFi.softAP("Energy Meter", NULL,10,false,2);
...
{
  {
    ...
    server.on("/setting", []() {
      ...
      if ((qsid.length() > 0) && (qpass.length() > 0) && (qemail.length() > 0) && (qupass.length() > 0)) {
        ...
        Serial.println(F("writing eeprom email:"));
        ...
        Serial.println(F("writing eeprom user password:"));
...
{  // This code is the setup() function in esp32 programming.
  ...
  Serial.println(F("Start ATM90E32")); // prints "Start ATM90E32" to the serial output
  ...
  Serial.println(F("Reading EEPROM email"));
 ...
 int connectionAttempts = 0;
  while ((WiFi.status() != WL_CONNECTED)) // while loop will repeatedly check the status of the Wi-Fi connection
  {
    ...
    Serial.print(F(".")); //  a dot is printed to the serial monitor to indicate that the code is still attempting to connect to Wi-Fi.
  ...
  */
  ...
  Home_importunits = values[3];
   ...
   Serial.println("Local Device IP: ");
   Serial.println(WiFi.localIP());
 ...
 if (millis() - startTime >= 5500) 
  {
        ...
        firebaseFlag = 1;
          ...
          Serial.println(timeStamp);
            ...
            String verPath = "Firmware_Version/"+uid+"/version";
            json.set("/Grid_voltage_A", String(Acin_voltageA));
            json.set("/Grid_voltage_B", String(Acin_voltageB));
            json.set("/Grid_voltage_C", String(Acin_voltageC));
            json.set("/Grid_current_A", String(Acin_currentA));
            json.set("/Grid_current_B", String(Acin_currentB));
            json.set("/Grid_current_C", String(Acin_currentC));
            json.set("/Grid_power", String(Acin_realPower));    
            json.set("/Inverter_voltage_A", String(Acout_voltageA));
            json.set("/Inverter_voltage_B", String(Acout_voltageB));
            json.set("/Inverter_voltage_C", String(Acout_voltageC));  
            json.set("/Inverter_current_A", String(Acout_currentA));
            json.set("/Inverter_current_B", String(Acout_currentB));
            json.set("/Inverter_current_C", String(Acout_currentC));
            json.set("/Inverter_power", String(Acout_realPower));
            json.set("/Grid_Power_Factor", String(Acin_powerFactor));
            json.set("/Inverter_Power_Factor", String(Acout_powerFactor));
            json.set("/Export_Units", String(Acin_exportunits));
            json.set("/Import_Units", String(Acin_importunits));  
            json.set("/Units_Generated", String(Acout_importunits));
            json.set("/Home_voltage_A", String(Home_voltageA));
            json.set("/Home_voltage_B", String(Home_voltageB));
            json.set("/Home_voltage_C", String(Home_voltageC));
            json.set("/Home_current_A", String(Home_currentA));
            json.set("/Home_current_B", String(Home_currentB));
            json.set("/Home_current_C", String(Home_currentC));
            json.set("/Frequency", String(Acout_freq));
            json.set("/Frequency", String(Acin_freq));
            json.set("/Temperature", String(temp));
            json.set("/Home_Power", String(Home_realPower));
            json.set("/Home_Consumption", String(Home_importunits));
            json.set("/Battery_Voltage", String("-"));
            json.set("/Battery_Current", String("-"));
            json.set("/Battery_Power", String("-"));
            Firebase.RTDB.set(&fbdo, "Firmware_Version/Update_Status/Device_IP",WiFi.localIP().toString());
            Firebase.RTDB.set(&fbdo, verPath,String(version));
            Firebase.RTDB.set(&fbdo, "Firmware_Version/Update_Status/status",false);
  ...
  String versionPath = "Firmware_Version/Update_Status/version";
  if(Firebase.RTDB.getString(&fbdo, versionPath.c_str()))
  {
     String verstr = fbdo.stringData();
     version = verstr.toFloat();
  }
    delay(2000);
...
bool IsFirmTrue()
{
  String FirmPath = "Firmware_Version/Update_Status/Update";
  String userPath = "Firmware_Version/Update_Status/User";
  if(Firebase.RTDB.getString(&fbdo, userPath.c_str()))
  {
     userUID = fbdo.stringData();
  }

  if (Firebase.RTDB.getString(&fbdo, FirmPath.c_str())) {
    String activeDataString = fbdo.stringData();
    bool activeData = activeDataString.equals("true");
    Serial.print(version);
    Serial.print(F(" Firm (bool): "));
    Serial.println(activeData);
...
void CheckUpdate()
{
  if(IsFirmTrue() == true && userUID == uid)
  {

    Firebase.RTDB.set(&fbdo, "Firmware_Version/Update_Status/Update",false);
    firmwareUpdate();
    HTTPUpdate state;
  if(state.OnUpdate())
  {
    Serial.println(F("Update Completed You can Now restart Device"));
    Firebase.RTDB.set(&fbdo, "Firmware_Version/Update_Status/status",true);
    version = version + 0.1;
    Firebase.RTDB.set(&fbdo, "Firmware_Version/Update_Status/version",String(version));
    delay(30000);
    ESP.restart();
  }
  }
